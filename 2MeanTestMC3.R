##### 2MeanTestMC3 #####
twoMeanTestMC3= function(
title = "2MeanTestMC3", # Question-bank title that will be easily viewable in e-learning
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "Is the p-value bigger or smaller than 0.05?", # This is a student hint, visible to them during the exam on e-learning
feedback = "Reject when p-value <= 0.05." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  dec <- sample(1:2, 1)
  if(dec == 1){data1 <- round(sample(seq(0.0001, .05, 10^-digits), size = 1), digits)} #generating p-value smaller than 0.05
  else{data1 <- round(sample(seq(0.06, 1, 10^-digits), size = 1), digits)} #generating p-value greater than 0.05
  corr.ans <- if(data1 > 0.05){"Fail to reject the null hypothesis."}
  else{"Reject the null hypothesis."} # this is the correct answer to the question based on the condition
...
twoMeanTestMC3() # creating the csv file