##### MeanCIMC1 #####
MeanCIMC1= function(
title = "MeanCIMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt4 = " inches. What is the 95% confidence interval for the average height of all US citizens?", # The above 4 question texts are static texts for the full question
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "Use your confidence interval formula for one sample mean questions. Pick the closest answer.", # This is a student hint, visible to them during the exam on e-learning
feedback = "Did you use xbar - 1.96*s/sqrt(n), and xbar + 1.96*s/sqrt(n)?" # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  data1 <- sample(seq(20,40), size = 1) # randomly generating data of size 1 for sample size for the calculation
  data2 <- sample(seq(54,66), size = 1) #randomly generating data of size 1 for mean for the calculation
  data3 <- sample(seq(3,18), size = 1) # randomly generating data of size 1 for sample standard deviation for the calculation
...
MeanCIMC1() # creating the csv file