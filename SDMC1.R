##### SDMC1 #####
SDMC1= function(
title = "SDMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt = "What is the standard deviation of the following dataset?  Data: ", # This is static text for the question
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "Don't forget to take a square root at the end.", # This is a student hint, visible to them during the exam on e-learning
...
for(i in 1:n)
{
  ...
  data <- round(rnorm(dat.size,mean=rnorm(1,mean=900,sd=400),sd=200) + (0.5*rt(dat.size,df=30)), digits = 1) # This is how we're randomly generating data for this question
  corr.ans <- round(sd(data), digits = digits) # This is the correct answer to the question
