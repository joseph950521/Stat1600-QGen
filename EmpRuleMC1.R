##### EmpRuleMC1 #####
EmpRuleMC1= function(
title = "EmpRuleMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt5 = "% of all the data?", # the above 5 question texts are static text for the full question
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "This is an empirical rule question.", # This is a student hint, visible to them during the exam on e-learning
feedback = "1: Determine if the endpoint is lower or upper. 2: Find the probability centered on the mean (68, 95, or 99.7%). 3: Subtract or add 1, 2, or 3 SDs from the mean depending on steps 1 and 2." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  data1 <- round((rnorm(dat.size,mean=rnorm(1,mean=900,sd=300),sd=100) + (0.25*rt(dat.size,df=30))), digits = digits) # randomly generating mean value for the question
  data2 <- round(runif(dat.size, 65, 150), digits = digits) # randomly generating standard deviation for the question
  data3 <- sample(c("lower", "upper"), size = 1) # generating endpoint options for the question 
  data4 <- sample(c(68, 95, 99.7), size = 1) # randomly generating any one of three probability cenetered on mean
  corr.ans <- ifelse((data3 == "lower"), (data1 - data2*if(data4 == 68){1}
                                          else{if(data4 == 95){2}
                                            else{3}}),
                     (data1 + data2*if(data4 == 68){1}
                      else{if(data4 == 95){2}
                        else{3}})) # This is the correct answer to the question
...
EmpRuleMC1() # creating the csv file