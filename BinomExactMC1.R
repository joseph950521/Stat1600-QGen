##### BinomExactMC1 #####
BinomExactMC1= function(
title = "BinomExactMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt3 = " questions correctly?",# The above 3 question texts are static texts for the full question
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "First, you need to find the number of successes, the number of trials, and infer the probability p from the context of this question. Round and select the closest answer.", # This is a student hint, visible to them during the exam on e-learning
feedback = "1: Find successes x. 2: Find trials n. 3: Infer p = 0.5. 4: Apply exact binomial formula." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  data1 <- sample(10:20, size = 1) # randomly generating number of trials needed for the question 
  data2 <- sample(1:7, size = 1) # randomy generating the number of successes needed for the question
  corr.ans <- round(dbinom(data2, data1, 0.5), digits = digits) # this is the correct answer to the question based on the exact binomial formula
...
BinomExactMC1() # creating the csv file