...
##### 2MeanTestMC2 #####
twoMeanTestMC2= function(
title = "2MeanTestMC2", # Question-bank title that will be easily viewable in e-learning
...
quest.txt7 = ". What is the P-Value of a hypothesis test on the difference between the two means (first group - second group)?", # The above 7 question texts are static texts for the full question
...
loc.path , # This is the local path used to store any randomly generated image files
  ...
  hint = "You need to calculate the two-tailed P-Value. Pick the closest answer.", # This is a student hint, visible to them during the exam on e-learning
feedback = "Did you calculate Z = (xbar1 - xbar2)/SE, take the absolute value, then calculate twice the area above this value?" # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  corr.ans <- round(2*pnorm(round(abs((data3 - data5)/sqrt(data4^2/data1 + data6^2/data2)), digits = 2), lower.tail = F), digits) # this is the correct answer to the question
...
twoMeanTestMC2() # creating the csv file