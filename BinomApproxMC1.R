##### BinomApproxMC1 #####
BinomApproxMC1= function(
title = "BinomApproxMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt4 = " bottles of beer in this particular sample are defective?", # The above 4 question texts are static texts for the full question
...
feedback = "1: Find successes x. 2: Find trials n. 3. Find probability p. 4. Use the normal approximation formula with appropriate continuity correction. 5. Use the z table to find the probability." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  data1 <- sample(900:1100, size = 1) # randomly generating sample size for the question
  data2 <- sample(seq(.11, .16, .01), size = 1) # randomly generating population probability for the question
  decis1 <- sample(125:160, size = 1) # randomly generating the number of successes for the question
...
BinomApproxMC1() # creating the csv file