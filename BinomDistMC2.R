##### BinomDistMC2 #####
BinomDistMC2= function(
title = "BinomDistMC2", # Question-bank title that will be easily viewable in e-learning
...
for(i in 1:n)
{
  ...
  data1 <- sample(6:7, size = 1) # randomly generating number of trials needed for the question 
  data2 <- sample(3:6, size = 1) # randomly generating the number of answers per question for the given problem
  decis1 <- sample(3:4, size = 1) # randomy generating the number of successes needed for the question
  decis2 <- sample(c("or more", "greater than"), size = 1) # generating decision vector for the question
...
BinomDistMC2() # creating the csv file