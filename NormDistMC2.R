##### NormDistMC2 #####
NormDistMC2= function(
title = "NormDistMC2", # Question-bank title that will be easily viewable in e-learning
...
feedback = "1: Calculate Z. 2: Find area below on the Z-table. 3: Take 1 - area below." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  corr.ans <- 1 - pnorm(round(((data3-data1)/data2), digits = 2)) # This is the correct answer to the question
  up.min <- round(corr.ans + .05, digits = digits) # This is the minimum value for incorrect answers above the correct answer
  down.max <- round(corr.ans - .05, digits = digits) # This is the maximum value for incorrect answers below the correct answer
...
NormDistMC2() # creating the csv file
