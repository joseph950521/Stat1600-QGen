#### PropDistMC1 #####
PropDistMC1 = function(
title = "PropDistMC1", # Question-bank title that will be easily viewable in e-learning
...
quest.txt4 = " of her own sample study for 4 or more hours per day, what is the probability of observing this sample proportion or less?", # The above 4 question texts are static texts for the full question
...
feedback = "Z = (phat - p)/sqrt(pq/n). The probability can be found directly from the Z table." # This is student feedback, visible after the exam
)
{
...
for(i in 1:n)
{
  ...
  corr.ans <- round(pnorm(round((data3-data2)/sqrt(data2*(1-data2)/data1), digits = digits)), digits = digits) # this is the correct answer to the question
...
PropDistMC1() # creating the csv file